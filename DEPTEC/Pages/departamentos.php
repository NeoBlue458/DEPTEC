...
$SQL="SELECT departamento.id_departamento, departamento.nombre_D, departamento.amueblado,
departamento.precio FROM departamento";
...
?>
<tr>
    <!-- colspan="5" asegura que el mensaje ocupe todo el ancho de la tabla -->
    <td colspan="5" style="text-align: center; color: #f8d7da; background-color: #721c24; padding: 10px; font-weight: bold;">
        ⚠️ No se encontraron registros de personal con la solicitud actual.
    </td>
</tr>
<?php
} // <-- Cierra el bloque if
...
?>
      ...
      <div class="actions">
        <form action="insert_departamentos.php" method="POST">
...
      </div>
  ...
  <div id="modalAgregar" class="modal" role="dialog" aria-modal="true">
    <div class="modal-content">
      ...
      <form action="insert_departamentos.php" method="POST" id="formAgregar">
        <div class="inputs">
          ...
          <div class="checkbox-group">
            <input type="checkbox" id="roperoAgregar" name="amueblado" value="Ropero">
            <label for="roperoAgregar">Ropero</label>

            <input type="checkbox" id="camaAgregar" name="amueblado" value="Base de Cama">
            <label for="camaAgregar">Base de Cama</label>

            <input type="checkbox" id="sillonAgregar" name="amueblado" value="Sillón">
            <label for="sillonAgregar">Sillón</label>

            <input type="checkbox" id="mesaAgregar" name="amueblado" value="Mesa de Noche">
  ...
  <script>
   ...
   $(document).ready(function() {
    ...
    // 1. Manejo de la SELECCIÓN de Fila
    $('.fila-departamento').on('click', function() {
    ...
    $('#btnEditar').on('click', function() {
        if (!idSeleccionado) return; // No debería suceder si está deshabilitado

        // **Paso AJAX para EDITAR**
        $.ajax({
            url: 'obtener_registro.php', // Tu script PHP que devuelve los datos en JSON
            type: 'POST',
            data: { id: idSeleccionado },
            dataType: 'json',
            success: function(data) {
                // Llenar el formulario dentro del modal con los datos (data)
                $('#modalEditar').find('#id_registro_oculto').val(data.Id);
                $('#modalEditar').find('#input_nombre').val(data.Nombre);
                // ... llenar otros campos ...
                
                // El modal se abrirá automáticamente por los atributos de Bootstrap
            },
            error: function() {
    ...
    $('#btnEliminar').on('click', function() {
        ...
        if (confirm('¿Está seguro de que desea eliminar el departamento ' + idSeleccionado + '? Esta acción es irreversible.')) {
            
            // **Paso AJAX para ELIMINAR**
            $.ajax({
                url: 'eliminar_registro.php', // Tu script PHP que ejecuta el DELETE
                type: 'POST',
                data: { id: idSeleccionado },
                dataType: 'json', // Asumiendo que PHP devuelve JSON
